...
final class ChatbotConversationMaterializeOutput extends AbstractChatbotConversationOutput {

	public static function getName(): string {
		return 'chatbotconversationmaterialize';
	}

	protected function handle(array $input): array {
		$state = $this->conversationService->materializeConversation(
			$this->requireString($input, 'config_group'),
			$this->requireString($input, 'config_name'),
			$this->requireString($input, 'draft_id'),
